...
class SoftwareUpdateChanges(Screen):
	...
	def getlog(self):
		global ocram
		try:
			sourcefile = 'http://www.openvix.co.uk/feeds/' + getImageVersion() + '/' + self.logtype + '-git.log'
		...
		if '404 Not Found' not in releasenotes:
			releasenotes = releasenotes.replace('[openvixhd] Apollo-HD Release.', 'openvixhd: build 000')
