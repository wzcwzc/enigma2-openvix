...
#
	def buildTimerEntry(self, timer, processed):
		...
		res = [ None ]
		x = width / 2
		res.append((eListboxPythonMultiContent.TYPE_TEXT, 50, 1, width, 35, 0, RT_HALIGN_LEFT|RT_VALIGN_BOTTOM, timertype))
		if timer.timerType == TIMERTYPE.AUTOSTANDBY or timer.timerType == TIMERTYPE.AUTODEEPSTANDBY:
			...
			if not processed:
				...
				state = _("done!")
				icon = self.iconDone
			res.append((eListboxPythonMultiContent.TYPE_TEXT, 148, 37, width-150, 35, 1, RT_HALIGN_RIGHT|RT_VALIGN_BOTTOM, _("Delay:") + " " + str(timer.autosleepdelay) + "(" + _("mins") + ")"))
		else:
			res.append((eListboxPythonMultiContent.TYPE_TEXT, x+24, 2, x-2-24, 35, 1, RT_HALIGN_RIGHT|RT_VALIGN_BOTTOM, _('At End:') + ' ' + afterevent))
			...
			res.append((eListboxPythonMultiContent.TYPE_TEXT, 148, 37, width-150, 35, 1, RT_HALIGN_RIGHT|RT_VALIGN_BOTTOM, text))
			icon = None
			if not processed:
				if timer.state == TimerEntry.StateWaiting:
					state = _("waiting")
		...
		res.append((eListboxPythonMultiContent.TYPE_TEXT, 50, 37, 126, 25, 1, RT_HALIGN_LEFT|RT_VALIGN_TOP, state))
		if icon:
